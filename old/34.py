...
def signal_handler(signum, frame):
    ...
    if global_ser and global_ser.isOpen():
        global_ser.close()
        print("机械臂串口已关闭")
    if portHandler and portHandler.isPortOpen():
...
def on_aruco_marker_detected(ser, marker_id):
    ...
    def process_marker():
        ...
        # 第一次识别到34时执行抓取
        if marker_detection_count == 0 and (marker_id == '3' or marker_id == '4'):
            # 保证机械臂爪子在原位
        ...
        if current_time - last_command_time > 5 and marker_detection_count !=0 and (marker_id == '3' or marker_id == '4'):
            if marker_id == '3' and current_time - last_command_time > 12:
                time.sleep(2)
                # 向右机械臂爪子放
                send_data_to_serial(ser, json_commands_set1)
                marker_detection_count = marker_detection_count + 1
                last_command_time = current_time   # 更新最后执行指令的时间
            elif marker_id == '4' and current_time - last_command_time > 15:
                time.sleep(2)
                # 向右下去(throw)
...
try:
    while True:
        ...
        json_commands_set1 = [
            ...
            {"T": 122, "b": -90, "s": 100, "e": 65, "h": 155, "spd": 90, "acc": 10},
        ...
        json_commands_set5 = [
            {"T": 121, "joint": 1, "angle": -90, "spd": 90, "acc": 10},
            {"T": 122, "b": -90, "s": 80, "e": 90, "h": 201, "spd": 90, "acc": 10}
        ]
        ...
        json_commands_set7 = [
            ...
            {"T": 122, "b": -90, "s": 100, "e": 65, "h": 155, "spd": 100, "acc": 10},
        ]
