...
def set_setpoint(self, setpoint):
        ...
        self.previous_error = 0  # 重置上一次误差
target_angle_degrees = 0
angle_pid = PID(Kp=1.0, Ki=0.0, Kd=0.1, setpoint=target_angle_degrees)
    ...
    # 在图像上绘制检测到的 ArUco 码及其 ID
        ...
        if left_roundabout < 1 or left_roundabout == out_left_turnabout == 1 or left_roundabout == out_left_turnabout == 2 and right_roundabout != 1 and right_roundabout != 3 or right_roundabout == 2 or right_roundabout == 4:
            ...
            if len(mid_points) > 1:
                ...
                # 计算偏角（以度为单位）
                angle =90-math.atan2(y2 - y1, x2 - x1) * 180 / np.pi 
                adjusted_angle = angle_pid.update(angle)* np.pi / 180
                # 计算横向偏移（直线中点与图像中心的距离）
                line_center = (x1 + x2) / 2
        ...
        float_number_3 = 0
        message_to_send = "{:.2f}|{:.2f}|{:.2f}|{}".format(adjusted_angle, float_number_2, float_number_3, marker_id_str)
