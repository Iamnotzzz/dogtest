...
# 实例化 PD 控制器
pd_angle = PD(Kp_angle, Kd_angle)
pd_offset = PD(Kp_offset, Kd_offset)
jishu = 0
angle_output = 0
offset_output = 0
add_line = False
    ...
    # 在图像上绘制检测到的 ArUco 码及其 ID
        ...
        if left_roundabout < 1 or left_roundabout == out_left_turnabout == 1 or left_roundabout == out_left_turnabout == 2 and right_roundabout != 1 and right_roundabout != 3 or right_roundabout == 2 or right_roundabout == 4:
            ...
            if len(mid_points) > 1:
                ...
                angle_output = pd_angle.compute(0, angle)  # 设定点为 0，即希望偏角为 0
                offset_output = pd_offset.compute(0, lateral_offset)  # 设定点为 0，即希望横向偏移为 0
        	
                # 计算横向偏移（直线中点与图像中心的距离）
                line_center = (x1 + x2) / 2
                image_center = width / 2
                lateral_offset = line_center - image_center                # 在图像上显示偏角和横向偏移
                angle_output = pd_angle.compute(0, 1.57-angle)  # 设定点为 0，即希望偏角为 0
                offset_output = pd_offset.compute(0, lateral_offset)  # 设定点为 0，即希望横向偏移为 0
                cv2.putText(frame, f"Angle: {angle:.2f} degrees", (25, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0),1)
                
                
                cv2.putText(frame, f"Lateral Offset: {lateral_offset:.2f} pixels", (25, 50), cv2.FONT_HERSHEY_SIMPLEX,0.5,(0, 255, 0), 1)
        ...
        float_number_3 = 0
        message_to_send = "{:.2f}|{:.2f}|{:.2f}|{}".format(angle_output, float_number_2, float_number_3, marker_id_str)
